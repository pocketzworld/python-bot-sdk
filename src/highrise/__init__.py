...
class Highrise:
    ...
    async def equip(self: BaseBot, user: User, message: str):
        ...
        if item == []:
            await self.chat(f"Item '{item_name}' not found.")
            return
        elif len(item) > 1:
            await self.chat(f"Multiple items found for '{item_name}', using the item number {index} in the list {item[index].item_name}.")
        item = item[index]
        ...
        inventory = (await self.get_inventory()).items
        ...
        if verification == False:
            ...
            #checks if the item is purchasable
                ...
                await self.chat(f"Item '{item_name}' can't be purchased.")
                return
            else:
                try:
                    ...
                    if response != "success":
                        ...
                        await self.chat(f"Item '{item_name}' purchased.")
                except Exception as e:
                    print(e)
                    await self.chat(f"Exception: {e}'.")
                    return
        ...
        outfit = (await self.get_my_outfit()).outfit
        items_to_remove = []
